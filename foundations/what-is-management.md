...
* **Multiplier.** An individual contributor is successful when *they* get results. A manager is successful when *each of their reports* — and the team as a whole — gets results. This is the hardest shift for new managers coming from senior IC roles.
...
{: .warning }
> **The servant-leadership trap.** Resist the urge to take over when problems arise. "Don't worry, I'll handle it" is satisfying to say and destructive to do. Your job is to help your team solve the problem themselves, not to solve it for them. The short-term win costs you a long-term contributor.
...
The goal isn't to stay as technical as a senior engineer. The goal is to stay technical enough that you can tell when an engineer is sandbagging, when an estimate is fantasy, and when the team is stuck on something that isn't actually the hard problem.
...
2. **Delivery.** You are accountable for what the team ships and when. Not what gets built (that's Product's call) but who builds it, how, and when it lands.
...
Supporting those three are two operational areas every EM has to be conversant in: **org design** (how you shape the team to match the work) and **infrastructure** (what you need to know about CI/CD and security to not get blindsided).
